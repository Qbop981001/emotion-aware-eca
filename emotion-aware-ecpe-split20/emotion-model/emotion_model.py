...
class EmotionContextModel(nn.Module):
    def __init__(self):
        super(EmotionContextModel,self).__init__()
    ...
    def BCEloss(self, pred_logit, emotions, doc_lens):
        ...
        # print(f"loss shape {pred_logit.shape},{emotions.shape},{mask.shape}")
        if pred_logit.size()[1] != emotions.size()[1]:
            print("Long document, but error!")
            print(emotions)
            emotions = emotions[:, :pred_logit.size()[1]]
        criterion = nn.BCEWithLogitsLoss(reduction='mean')
        pred_logit = pred_logit.masked_select(mask)

        emotions = emotions.masked_select(mask)
        loss = criterion(pred_logit,emotions.float())
        return loss
    ...
    def cls_output(self,sequence_output,clauses_positions,doc_lens):  # 失败的尝试 直接使用[CLS]根本不行
