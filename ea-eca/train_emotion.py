...
def main(fold_id, train_loader, valid_loader):
    ...
    for epoch in range(EPOCH):
        ...
        r = out[1][1]
        f1 = out[2][1]
        epoch_metric = f"acc={acc:.5f}, p={p:.5f}, r={r:.5f}, f1={f1:.5f}."
...
if __name__ == '__main__':
    ...
    for fold_id in range(1, n_folds+1):
        ...
        train_loader = build_dataloader(fold_id, 'train', 'sentiment')
        valid_loader = build_dataloader(fold_id, 'test', 'sentiment')
        acc, p, r, f1 = main(fold_id,train_loader,valid_loader)
        metric_str = metric = f"Best of fold{fold_id}: acc={acc:.5f}, p={p:.5f}, r={r:.5f}, f1={f1:.5f}."
        print(metric_str)
        with open('sentiment_classification_result.txt','a') as f:
            f.write(metric_str+'\n\n')
