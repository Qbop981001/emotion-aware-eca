...
def main(fold_id,train_loader,valid_loader,real_valid_loader):
    ...
    for epoch in range(EPOCH):
        ...
        p = metric[0][0]; r = metric[0][1] ; f1 = 2*p*r/(p+r)
        ece = metric[1]
        real_metric = metric[2]
        out_str = f"p = {p}, r = {r}, f1 = {f1}  ece = {ece}   real: {real_metric} epoch_time: {epoch_time}s"
        print(out_str)
    ...
    with open(os.path.join(result_dir,'fold'+str(fold_id)+'results2e.txt'),'w') as f:
        for i in out_strs:
            f.write(i+'\n')
...
def predict(fold_id,valid_loader,real_valid_loader):
    model = BertForQuestionAnswering.from_pretrained("bert-base-chinese")
    model.to(device)
    model.load_state_dict(torch.load(f"checkpoint/fold{fold_id}_best.pth"))
    metric = evaluate(model, valid_loader, real_valid_loader, device)
    p = metric[0][0]
    r = metric[0][1]
    f1 = 2 * p * r / (p + r +1e-8)
...
if __name__ == '__main__':
    ...
    for fold_id in range(1, 11):
        ...
        best_str = f"best f1: {metric_ece[0]}, in epoch {metric_ece[1]}; best acc: {metric_ece[2]}, in epoch {metric_ece[3]}, real_f1_best: {metric_ece[4]} n epoch {metric_ece[5]}"
        print(best_str)
        with open(os.path.join(result_dir,'fold'+str(fold_id)+'results2e.txt'), 'a') as f:
            f.write(best_str+'\n')
    ...
    with open('2e-5-42-0.2result', 'w') as f:
        f.write(f"average_f1: {average_f1}, average_acc: {average_acc}, average_real: {average_real}")
